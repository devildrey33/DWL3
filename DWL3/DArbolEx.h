...
#endif
	// Objeto con la configuración de colores y fuente por defecto para el arbol
	class DArbolEx_Skin {
	  public :
						DArbolEx_Skin(void);
	...
	class DArbolEx : public virtual DBarraScrollEx {
	  public: //////////////////////////////////////////// Constructor
		...
		inline const size_t								TotalNodos(void)		{ return _Raiz._Hijos.size(); }
														// Función que devuelve el total de nodos seleccionados
		...
		virtual void									Evento_MousePresionado(DEventoMouse &DatosMouse)											{ };
														// Función lambda que responde a los mensajes WM_?MOUSEDOWN
		...
		virtual void									Evento_MouseSoltado(DEventoMouse &DatosMouse)												{ };
														// Función lambda que responde a los mensajes WM_?MOUSEUP
		...
		virtual void                                    Evento_TeclaPresionada(DEventoTeclado &DatosTeclado)										{ };
														// Función lambda que responde al mensaje WM_KEYDWON
		...
		virtual void									Evento_Nodo_Expandido(DWL::DArbolEx_Nodo *nNodo, const BOOL nExpandido)						{ };
														// Función que devuelve el nodo en la posición especificada
		DArbolEx_Nodo                                  *HitTest(const int cX, const int cY, DArbolEx_ParteNodo &Parte);
														// Función que muestra el nodo especificado
		void											MostrarNodo(DArbolEx_Nodo *vNodo);
														// Función que devuelve el área del nodo
		const BOOL										ObtenerRectaNodo(DArbolEx_Nodo *rNodo, RECT &rRecta);
														// Función que selecciona / desselecciona el nodo especificado
		...
		void											SeleccionarNodosShift(DArbolEx_Nodo *Desde, DArbolEx_Nodo *Hasta);
														// Variable que permite habilitar / des-habilitar la multiselección
		BOOL											MultiSeleccion;
														// Variable que permite habilitar / des-habilitar la subsleección
		BOOL											SubSeleccion;
														// Función que des-selecciona todos los nodos
		...
		DArbolEx_Skin									Skin;
														// Función que actualiza todos los nodos del arbol en base al objeto Skin
	  ...
	  protected: ///////////////////////////////////////// Expansor para el arbol		
		...
		void                                           _Evento_MouseRueda(WPARAM wParam, LPARAM lParam);
														// Función que responde al mensaje WM_MOUSEDBLCLICK
		...
		void										   _Evento_FocoPerdido(HWND hWndNuevoFoco);
														// Función que aplica el shift desde el nodo marcado al nodo resaltado
		void                                           _AplicarShift(const LONG nPosShift);
														// Función que crea el buffer de pintado para un nodo
		...
		void										   _Tecla_RePag(const BOOL TeclaShift);
														// Función que calcula los nodos que caben en la página
	    void					  					   _CalcularNodosPagina(const size_t TamPagina);
														// Función que calcula las barras de scroll
		...
		DArbolEx_Nodo                                 *_NodoPaginaFin;
														// Diferencia de pixeles vertical del nodo inicial y el principio de la página (o es 0 o es un valor negativo)
		LONG										   _NodoPaginaVDif;
														// Diferencia de pixeles horizontal del nodo inicial y el principio de la página (o es 0 o es un valor negativo)
		...
		ULONGLONG                                      _TiempoTecladoTmp;
														// Función que comprara 2 strings para ver si son iguales (solo se compara hasta el tamaño del string mas pequeño)
