...
namespace DWL {
	...
	DDlgDirectoriosEx::DDlgDirectoriosEx(void) : _Padre(NULL), _Terminado(FALSE){
	}


	DDlgDirectoriosEx::~DDlgDirectoriosEx(void) {
	}


	const BOOL DDlgDirectoriosEx::Mostrar(DhWnd *nPadre, std::wstring &rPath, const int cX, const int cY, const int cAncho, const int cAlto, const int ID_Icono) {
		_Padre = nPadre;
		_Terminado = FALSE;
		// Creo la ventana
		...
		EdicionSeleccion.CrearEdicionTextoEx(this, L"", 10, RC.bottom - 80, RC.right - 20, 24, ID_EDICIONDIRECTORIO, WS_CHILD | WS_VISIBLE);
...
//		rIcono = EdicionSeleccion.Icono();
	...
	void DDlgDirectoriosEx::Evento_Cerrar(void) {
		_Terminado = 2; // cancelado
	}

	void DDlgDirectoriosEx::Evento_BotonEx_Mouse_Click(DWL::DEventoMouse &DatosMouse) {
	...
	void DDlgDirectoriosEx::Evento_BorraFondo(HDC DC) {
		RECT RC;
	...
	void DDlgDirectoriosEx::_ActualizarAceptar(void) {
	...
	void DDlgDirectoriosEx::Evento_CambiandoTam(const UINT Lado, RECT* Rectangulo) {
	...
	void DDlgDirectoriosEx::_AjustarControles(RECT& RC) {
	...
	LRESULT CALLBACK DDlgDirectoriosEx::GestorMensajes(UINT uMsg, WPARAM wParam, LPARAM lParam) {
		switch (uMsg) {
			case WM_ERASEBKGND:
